...
import { FiX, FiMonitor, FiTablet, FiSmartphone, FiMaximize2 } from 'react-icons/fi';

export default function FormPreviewModal({ isOpen, onClose, formData, type = 'form' }) {
  ...
  const renderField = (field) => {
    const inputClasses = "w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition";
          ...
          <div key={field.id} className="mb-4">
            ...
            <div className="flex gap-2">
              {[...Array(field.max || 5)].map((_, idx) => (
                <span key={idx} className="text-2xl text-gray-300 cursor-not-allowed">
                  ⭐
                </span>
              ))}
            </div>
  ...
  const renderPreview = () => {
    ...
    console.log("🔍 formData.custom_code length:", formData.custom_code?.length);
    ...
    if (formData.type === 'custom' && formData.custom_code) {
      ...
      return (
        <div className="w-full h-full">
          ...
          <div className="relative w-full h-full bg-white rounded-xl shadow-2xl overflow-hidden border-8 border-gray-800" 
               style={{ 
    ...
    if (type === 'popup') {
      return (
        <div className="bg-white rounded-lg shadow-xl p-6 max-w-md mx-auto relative">
          ...
          {formData.name && (
            <h3 className="text-xl font-bold text-gray-900 mb-3">{formData.name}</h3>
          )}
          {formData.content && (
            <div className="text-gray-700 mb-4" dangerouslySetInnerHTML={{ __html: formData.content }} />
          )}
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 bg-opacity-95 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className={`bg-white rounded-2xl shadow-2xl w-full flex flex-col overflow-hidden transition-all duration-300 ${
        isPreviewFullscreen ? 'max-w-full h-full m-0 rounded-none' : 'max-w-7xl max-h-[95vh]'
      }`}>
        {/* Header */}
        <div className="flex items-center justify-between px-8 py-5 border-b bg-gradient-to-r from-gray-50 to-white">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              {formData.type === 'custom' ? 'Custom HTML Form' : `${type.charAt(0).toUpperCase() + type.slice(1)} Preview`}
        ...
        <div className="px-8 py-4 border-t bg-gray-50 flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            <div className={`w-2 h-2 rounded-full ${device === 'desktop' ? 'bg-blue-500' : device === 'tablet' ? 'bg-purple-500' : 'bg-green-500'} animate-pulse`}></div>
            <span className="font-medium">
              {device === 'desktop' ? '1200px' : device === 'tablet' ? '768px' : '375px'} viewport
            </span>
          </div>
          <p className="text-xs text-gray-500 italic">
            This is a preview only. Actual appearance may vary based on your website's styling.
          </p>
        </div>
      </div>
    </div>
