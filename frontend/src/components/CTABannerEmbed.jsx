...
const CTABannerEmbed = ({ id }) => {
  ...
  const fetchBanner = async () => {
    try {
      ...
      if (response.data.status === "active") {
        ...
        // Track view
        api
          .post(`/cta-banners/${id}/track-view`)
  ...
  const handleClick = () => {
    if (banner) {
      // Track click
      api
        .post(`/cta-banners/${banner.id}/track-click`)
        .catch((err) => console.error("Failed to track click:", err));

      // Priority: form_id > button_url > default
      if (banner.form_id) {
        // Open form in modal
        window.dispatchEvent(
          new CustomEvent("openFormModal", {
            detail: { formId: banner.form_id },
          })
        );
      } else if (banner.button_url) {
        // Navigate to URL
        ...
        // Default action - navigate to contact
  ...
  // Convert database colors to component format
  const customColors = banner.colors
    ? {
        bgFrom: banner.colors.bgFrom || "#ef4444",
        bgTo: banner.colors.bgTo || "#dc2626",
        buttonBg: banner.colors.buttonBg || "#ffffff",
        buttonText: banner.colors.buttonText || "#dc2626",
        text: banner.colors.text || "#ffffff",
      }
    : null;
