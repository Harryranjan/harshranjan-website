...
(async () => {
  try {
    ...
    if (pages.length > 0) {
      ...
      console.log('\nIs Full HTML?', p.content && p.content.trim().toLowerCase().startsWith('<!doctype html') ? '✅ YES (iframe mode)' : '❌ NO');
      console.log('Has [form shortcode?', p.content && p.content.includes('[form') ? '✅ YES' : '❌ NO');
