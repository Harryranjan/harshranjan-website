...
async function removeJKTiwari() {
  ...
  const jkTiwariStart = content.indexOf(
    "<!-- Team Member 2: Dr. J.K. Tiwari -->"
  );
  ...
  if (jkTiwariStart === -1) {
    ...
    while (
      searchPos > 0 &&
      content
        .substring(searchPos - 200, searchPos)
        .indexOf(
    ...
    // Find the card start
    const cardStart = content.lastIndexOf(
      '<div class="bg-white rounded-2xl shadow-xl overflow-hidden',
      searchPos
    );
    ...
    // Remove the section
    content =
      content.substring(0, jkTiwariStart) + content.substring(jkTiwariEnd);
  }
