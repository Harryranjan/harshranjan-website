...
async function runTests() {
  ...
  if (
    await testFeature("Server is running on port 5000", async () => {
  ...
  if (
    await testFeature("Security Headers (Helmet.js)", async () => {
  ...
  if (
    await testFeature("API Compression (Gzip)", async () => {
      const res = await axios.get(`${API_URL}/blog`, {
  ...
  if (
    await testFeature("Input Sanitization", async () => {
  ...
  if (
    await testFeature("Database Indexes", async () => {
      const fs = require("fs");
      const path = require("path");
  ...
  if (
    await testFeature("Redis Caching (Optional)", async () => {
      const fs = require("fs");
      const path = require("path");
  ...
  if (
    await testFeature("Image Optimization", async () => {
      const fs = require("fs");
      const path = require("path");
  ...
  if (
    await testFeature("XML Sitemap Generation", async () => {
  ...
  if (
    await testFeature("Robots.txt", async () => {
  ...
  if (
    await testFeature("SEO Metadata Generation", async () => {
      const fs = require("fs");
      const path = require("path");
  ...
  if (
    await testFeature("Blog API", async () => {
      const res = await axios.get(`${API_URL}/blog`);
  ...
  if (
    await testFeature("Pages API", async () => {
