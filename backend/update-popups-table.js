...
async function updatePopupsTable() {
  try {
    ...
    await sequelize.query('ALTER TABLE popups CHANGE COLUMN message content TEXT');
    console.log('✅ Renamed message to content');

    // Add template column
    try {
      await sequelize.query('ALTER TABLE popups ADD COLUMN template VARCHAR(255) AFTER type');
    ...
    // Add icon column
    try {
      await sequelize.query('ALTER TABLE popups ADD COLUMN icon VARCHAR(255) AFTER template');
    ...
    await sequelize.query('ALTER TABLE popups CHANGE COLUMN conversions dismissals INT(11) DEFAULT 0');
    console.log('✅ Renamed conversions to dismissals');

    // Update type ENUM
    await sequelize.query('ALTER TABLE popups MODIFY COLUMN type ENUM("notification", "chat", "cookie_consent", "promo", "social_proof", "newsletter", "custom") DEFAULT "notification"');
    console.log('✅ Updated type ENUM');

    // Update trigger_type ENUM
    await sequelize.query('ALTER TABLE popups MODIFY COLUMN trigger_type ENUM("manual", "time", "scroll", "exit", "click", "immediate") DEFAULT "immediate"');
