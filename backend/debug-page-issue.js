  ...
  try {
    ...
    columns.forEach(col => {
      if (col.Field === 'template') {
        console.log(`Template field: ${col.Type}, Null: ${col.Null}, Default: ${col.Default || 'None'}`);
      }
    });
    
    // Check current page data
    console.log('\n📄 CURRENT PAGE DATA:');
    const [rows] = await connection.execute('SELECT id, title, slug, template, status FROM pages WHERE slug = ?', ['homepage-2025']);
    if (rows.length > 0) {
    ...
    const [updateResult] = await connection.execute('UPDATE pages SET template = ? WHERE slug = ?', ['custom', 'homepage-2025']);
    console.log('Affected rows:', updateResult.affectedRows);
    
    // Verify the update
    console.log('\n✅ VERIFICATION:');
    const [verifyRows] = await connection.execute('SELECT template FROM pages WHERE slug = ?', ['homepage-2025']);
    ...
    const [enumCheck] = await connection.execute("SHOW COLUMNS FROM pages LIKE 'template'");
