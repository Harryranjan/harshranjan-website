...
async function checkPageAPI() {
  ...
  try {
    ...
    const checks = [
      ...
      { name: 'Hero Section', pattern: /DIGITAL.*EXPERIENCES.*BEYOND REALITY/s },
    ...
    console.log(`Head Section: ${headStart !== -1 && headEnd !== -1 ? 'Present' : 'Missing'}`);
    console.log(`Body Section: ${bodyStart !== -1 && bodyEnd !== -1 ? 'Present' : 'Missing'}`);
