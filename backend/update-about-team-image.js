...
async function updateAboutTeamImage() {
  ...
  const targetPath = 'frontend/public/images/doctor/Dr.SubodhGettingcertifications4.jpg';
  ...
  try {
    ...
    // Get current content
    const [rows] = await conn.query('SELECT content FROM pages WHERE slug = ?', ['about-team']);
    ...
    const oldImagePattern = /src=["']([^"']*(?:transform_any_image|doctor)[^"']*\.(?:png|jpg|jpeg|webp))["']/gi;
    ...
    if (matches) {
      ...
      console.log('⚠️  No doctor image pattern found. Looking for any image...');
      // Try to find the specific transform image
      const transformPattern = /https:\/\/drsubodh\.harshranjan\.in\/images\/transform_any_image[^"'\s]*/gi;
