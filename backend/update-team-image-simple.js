...
async function updateAboutTeamImage() {
  ...
  try {
    ...
    const [rows] = await conn.query(
      "SELECT id, title, content FROM pages WHERE slug = ?",
      ["about-team"]
    );
    ...
    const oldUrl =
      "https://drsubodh.harshranjan.in/images/transform_any_image_with_ai_for_free_t53lvoea0zh2ahgjvshw.png";
    ...
    if (content.includes(oldUrl)) {
      ...
      console.log(
        "⚠️  Old URL not found, looking for any doctor/transform image..."
      );
      // Try broader pattern
      const pattern =
        /https?:\/\/[^"'\s]*(?:transform_any_image|doctor)[^"'\s]*/gi;
